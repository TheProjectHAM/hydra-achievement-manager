...
import { LanguageIcon, InfoIcon, SaveIcon, CheckIcon, PaletteIcon, UpdateIcon, SteamIcon } from './Icons';
import LocaleSettings from './settings/LanguageSettings';
import AboutSettings from './settings/AboutSettings';
import AppearanceSettings from './settings/AppearanceSettings';
import UpdateSettings from './settings/UpdateSettings';
import ApiSettings from './settings/ApiSettings';
import { DateFormat, TimeFormat, SidebarGameScale, ApiSource } from '../types';
import { useTheme, Theme } from '../contexts/ThemeContext';
import { useI18n, Language } from '../contexts/I18nContext';

type SubTabId = 'language' | 'appearance' | 'api' | 'updates' | 'about';
...
const SettingsContent: React.FC = () => {
  ...
  }, [selectedLanguage, selectedTheme, selectedDateFormat, selectedTimeFormat, selectedSidebarGameScale, selectedSidebarMarquee, selectedApi, steamApiKey, savedSettings]);
        ...
        <div className="flex items-center gap-1 flex-nowrap overflow-x-auto scrollbar-thin w-full" style={{whiteSpace: 'nowrap'}}>
          {SETTINGS_SUB_TABS.map((tab) => (
              <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-1 px-4 py-2 rounded-lg transition-colors duration-200 text-sm font-semibold h-10 overflow-hidden text-ellipsis whitespace-nowrap ${
                      activeSubTab === tab.id
